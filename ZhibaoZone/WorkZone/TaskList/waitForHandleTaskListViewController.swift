//
//  waitForHandleTaskListViewController.swift
//  ZhibaoZone
//
...
class waitForHandleTaskListViewController: UIViewController,UITableViewDelegate,UITableViewDataSource{
    ...
    func loadOrderDataFromServer(pages:Int) {
        //去除未完成的数据请求
        for task in waitForHandleTaskListViewController.requestCacheArr{
            task.cancel()
        }
        waitForHandleTaskListViewController.requestCacheArr.removeAll()
        //先删除重试按钮
...
//        }
        ...
        waitForHandleTaskListViewController.requestCacheArr.append(dataRequest)
}
