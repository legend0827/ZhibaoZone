//
//  NotQuoteYetViewController.swift
//  ZhibaoZone
//
...
class NotQuoteYetViewController: UIViewController,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
...
//    var previewTypes:[String] = []
    ...
    lazy var NotQuoteYetCollectionView:UICollectionView = {
    ...
    override func viewDidLoad() {
        ...
        //添加下拉刷新
        NotQuoteYetCollectionView.es.addPullToRefresh {
            [weak self] in
            self?.refresh()
        }
        //添加上拉加载
        NotQuoteYetCollectionView.es.addInfiniteScrolling {
            [weak self] in
            self?.loadMore()
        }
    ...
    private func loadMore() {
        DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) {
            ...
            if self.page <= self.totalPageCount{
                ...
                self.NotQuoteYetCollectionView.reloadData()
                self.NotQuoteYetCollectionView.es.stopLoadingMore()
            }else{
                self.NotQuoteYetCollectionView.es.noticeNoMoreData()
    ...
    func loadOrderDataFromServer(pages:Int,categoryType:orderListCategoryType) {
        //去除未完成的数据请求
        for task in NotQuoteYetViewController.requestCacheArr{
            task.cancel()
        }
        NotQuoteYetViewController.requestCacheArr.removeAll()
        ...
        let dataRequest = Alamofire.request(requestURL,method:.get, parameters:params as? [String:AnyObject],encoding: URLEncoding.default) .responseJSON{
            ...
            case true:
                if let value = responseObject.result.value{
                    ...
                    if json.count == 3 {
                        ...
                        if !self.view.subviews.contains(self.NotQuoteYetCollectionView) {
                            self.view.addSubview(self.NotQuoteYetCollectionView)
                            self.StopLoadingAnimation()
                        }
                        self.NotQuoteYetCollectionView.reloadData()
                        self.NotQuoteYetCollectionView.es.stopPullToRefresh()
                    }else{
                        if self.page == 1{
                            self.StopLoadingAnimation()
            ...
            case false:
                self.NotQuoteYetCollectionView.es.stopPullToRefresh()
                if self.view.subviews.contains(self.NotQuoteYetCollectionView) {
                    self.NotQuoteYetCollectionView.removeFromSuperview()
                }else{
                    self.StopLoadingAnimation()
                }
        ...
        NotQuoteYetViewController.requestCacheArr.append(dataRequest)
    }
