...
/// 普通时连线颜色
let CircleConnectLineNormalColor = UIColor.colorWithRgba(34, g: 178, b: 246, a: 1) //UIColor.colorWithRgba(239, g: 57, b: 5, a: 1) 
...
class CircleView: UIView {
    ...
    override func layoutSubviews() {
        ...
        (self.subviews as NSArray).enumerateObjects({ (object, idx, stop) in
            ...
            //设置tag->用于记录密码的单元
            (object as! Circle).tag = idx + 1
            (object as! Circle).frame = frame
        })
    }
    ...
    func connectCirclesInRect(_ rect: CGRect, color: UIColor) {
        ...
        for i in 0..<num! {
            //取出选中按钮
            let circle = self.circleSet![i] as! Circle
    ...
    func clipSubviewsWhenConnectInContext(_ ctx: CGContext,clip:Bool) {
        ...
        if clip {
            ...
            (subviews as NSArray).enumerateObjects({ (circle, idx, stop) in
                ...
                //确定剪裁的形状
                ctx.addEllipse(in: (circle as! Circle).frame)
    ...
    func getCircleState() -> CircleState {
        return (self.circleSet?.firstObject as! Circle).state!
    }
    ...
    func circleSetLastObjectWithState(_ state: CircleState) {
        ...
        (self.circleSet?.lastObject as! Circle).state = state
    }
    ...
    func gestureEndByTypeSettingWithGesture(_ gesture: NSString,length: CGFloat) {
        ...
        if Int(length) < CircleSetCountLeast {
            ...
            let gestureOne = CircleView.getGestureWithKey(gestureOneSaveKey)
            
            if gestureOne == nil {//未输入过少于4的手势
                // 记录第一次密码
                CircleView.saveGesture(gesture as String, key: gestureOneSaveKey)
                ...
                let equal = gesture.isEqual(CircleView.getGestureWithKey(gestureOneSaveKey)) //匹配两次手势
                ...
                if equal {
                    // 一致，存储密码
                    CircleView.saveGesture(gesture as String, key: gestureFinalSaveKey)
                } else {
                    //不一致 重绘回显
                    changeCircleInCircleSetWithState(CircleState.circleStateError)
                }
    ...
    func gestureEndByTypeLoginWithGesture(_ gesture: NSString, length:CGFloat) {
        ...
        let password = CircleView.getGestureWithKey(gestureFinalSaveKey)! as NSString
    ...
    func calAngleAndconnectTheJumpedCircle() {
        ...
        //取出最后一个对象
        let lastOne = circleSet?.lastObject as! Circle
        
        //倒数第二个
        let lastTwo = circleSet?.object(at: self.circleSet!.count - 2) as! Circle
     ...
     */
    func enumCircleSetToFindWhichSubviewContainTheCenterPoint(_ point: CGPoint) ->  Circle? {
        
        var centerCircle: Circle?
        
        for circle: Circle in subviews as! [Circle] {
    ...
    func getGestureResultFromCircleSet(_ circleSet: NSMutableArray) -> NSString {
        ...
        for circle: Circle in circleSetArr as! [Circle] {
            //遍历取tag拼接字符串 作为密码标识符
